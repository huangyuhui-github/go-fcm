...
// ErrInvalidProjectId occurs if API key is not set.
...
func NewClient(projectId string, credentialsFile string, opts ...Option) (*Client, error) {
	if projectId == "" {
		return nil, ErrInvalidProjectId
	}

	tp, err := newTokenProvider(credentialsFile)
	if err != nil {
		return nil, err
	}
...
func (c *Client) send(ctx context.Context, data []byte) (*Response, error) {
	...
	// add headers
	//req.Header.Add("Authorization", fmt.Sprintf("key=%s", c.projectId))
	// get bearer token
